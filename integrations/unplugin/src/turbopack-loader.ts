 ...
 *
 * Injects the UI Bridge WS URL global and a dynamic script loader into
...
export default function uiBridgeInjectLoader(
  this: { query: { port?: number } },
  content: string,
): string {
  const port =
    this.query?.port ?? parseInt(process.env.UI_BRIDGE_PORT ?? process.env.UIB_PORT ?? '7378', 10);
