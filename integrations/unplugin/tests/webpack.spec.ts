...
test.describe('annotation round-trip', () => {
  ...
  test('annotation created on the page is persisted to the correct location', async ({
    ...
    await input.fill('webpack integration check');
    ...
    expect(body.annotations.some((a) => a.comment === 'webpack integration check')).toBe(true);

    const ann = body.annotations.find((a) => a.comment === 'webpack integration check')!;
