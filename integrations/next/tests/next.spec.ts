...
const ANNOTATIONS_DIR = resolve(NEXT_DEMO_ROOT, '.design-bridge', 'annotations');
...
test.describe('annotation round-trip', () => {
  ...
  test('annotation created on the page is persisted to the correct location', async ({ page }) => {
    ...
    await input.fill('next integration check');
    ...
    expect(body.annotations.some((a) => a.comment === 'next integration check')).toBe(true);

    const ann = body.annotations.find((a) => a.comment === 'next integration check')!;
