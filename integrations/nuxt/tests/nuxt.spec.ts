/**
 * Integration tests for @ui-bridge/nuxt — Nuxt 3 module.
 *
...
const ANNOTATIONS_DIR = resolve(NUXT_DEMO_ROOT, '.ui-bridge', 'comments');
...
test('client script tag pointing at the UI Bridge server is present', async ({ page }) => {
  await page.goto('/');
...
test.describe('comment round-trip', () => {
  ...
  test('comment created on the page is persisted to the correct location', async ({ page }) => {
    ...
    await waInput.locator('textarea').fill('nuxt integration check');
    ...
    const ann = body.comments.find((a) => a.comments?.[0]?.text === 'nuxt integration check')!;
