/**
 * Astro integration tests for UI Bridge.
 *
 * These are *integration* tests — they verify the @ui-bridge/astro
 ...
 *  2. The client bundle is registered and boots (uib-comment element defined).
 *  3. The client bundle is served by the middleware.
 *  4. The UI Bridge WebSocket server is reachable.
...
const ANNOTATIONS_DIR = resolve(ASTRO_DEMO_ROOT, '.ui-bridge', 'comments');

const UIB_PORT = parseInt(process.env.UI_BRIDGE_PORT ?? '7378', 10);
...
test('serves the client bundle at /__ui-bridge/client.js', async ({ request }) => {
  const res = await request.get('http://localhost:4321/__ui-bridge/client.js');
