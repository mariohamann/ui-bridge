...
export default defineConfig({
  ...
  webServer: {
    ...
    // are fast.
    command: `cd ${__dirname} && node build.mjs && cd ${path.resolve(__dirname, '../client')} && node build.mjs && cd ${path.resolve(__dirname, '../vite-plugin')} && node build.mjs && cd ${demoDir} && pnpm dev`,
    cwd: demoDir,
