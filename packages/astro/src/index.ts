...
import { designBridge as designBridgeVitePlugin } from '@design-bridge/vite-plugin';
  ...
  return {
    ...
    hooks: {
      'astro:config:setup': ({ updateConfig, injectScript, command }) => {
        ...
        // This also includes code-inspector-plugin (bundled inside @design-bridge/vite-plugin)
        ...
        updateConfig({ vite: { plugins: [designBridgeVitePlugin(options) as any] } });
